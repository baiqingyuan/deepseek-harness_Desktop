...
- ✅ **iOS 风格界面**：自绘 iOS 导航栏（交通灯按钮 / 居中标题 / 端口徽章）+ 圆角窗口与投影，配色取自 Apple System Colors
...
- ✅ **内置更新检查**：启动后静默检查最新版（读取随 Release 发布的静态清单 `latest.json`，**不受 GitHub API 限流影响**），有新版本托盘气泡提示（点击即升级）；也可随时右键托盘 → **「检查更新…」**，自动下载安装包并引导覆盖安装
