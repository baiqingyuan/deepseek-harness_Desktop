...
- ✅ **简洁的 Windows 扁平界面**：自绘标题栏（左侧图标与标题 / 右侧本地端口 / 标准最小化·最大化·关闭按钮，悬停有反馈）+ 1px 柔和描边与投影；标题栏与描边**跟随网页主题**自动切换深浅色，双击标题栏全屏/退出全屏
...
4. 关闭窗口 = 最小化到系统托盘（服务继续运行）；**左键单击托盘图标**恢复窗口，右键只弹托盘菜单，「真正退出」才彻底停止
